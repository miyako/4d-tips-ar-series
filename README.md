...
`SetupDiGetClassDevs`にフラグ`DIGCF_PRESENT`を**渡さなければ**，デバイスマネージャーに登録されているだけの仮想ポートも情報を取得することができます。インタフェース情報は，デバイス名（例: "SMJ USB Device"）に対応するポート番号（例: "COM3"）を特定するために必要です。
